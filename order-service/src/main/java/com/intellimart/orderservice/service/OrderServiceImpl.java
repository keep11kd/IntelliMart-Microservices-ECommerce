...
import com.intellimart.orderservice.client.ProductClient; // Import ProductClient
...
import com.intellimart.orderservice.dto.ProductResponse; // Import ProductResponse
import com.intellimart.orderservice.dto.StockDecrementRequest; // Import StockDecrementRequest
...
@Slf4j
@Transactional // Ensures all methods are transactional, rolling back on exceptions
public class OrderServiceImpl implements OrderService {

    private final OrderRepository orderRepository;
    private final OrderItemRepository orderItemRepository;
    private final ShoppingCartClient shoppingCartClient;
    private final ProductClient productClient; // Inject ProductClient
     ...
     */
    @Override
    public OrderResponse placeOrder(OrderRequest orderRequest) throws InsufficientStockException {
        ...
        for (OrderItemRequest itemRequest : orderRequest.getOrderItems()) {
            ...
            if (product.getPrice().compareTo(itemRequest.getPriceAtPurchase()) != 0) {
                ...
                // You might choose to throw an error or use the actual product price here.
                // For this example, we proceed with the priceAtPurchase from the request.
            }
     ...
     * This method now includes inventory deduction.
     *
     * @param userId The ID of the user whose cart should be converted to an order.
     ...
     */
    public OrderResponse createOrderFromCart(String userId) throws ResourceNotFoundException, InsufficientStockException {
        log.info("Attempting to create order from cart for user ID: {}", userId);

        // 1. Fetch cart contents from shopping-cart-service
        CartResponse cart = shoppingCartClient.getCartByUserId(userId);

        if (cart == null || cart.getItems() == null || cart.getItems().isEmpty()) {
            log.warn("Cart is empty (or could not be retrieved) for user ID: {}", userId);
            throw new ResourceNotFoundException("Cannot create order: Cart is empty for user ID " + userId);
        }
        ...
        Order order = Order.builder()
                ...
                .shippingAddress("Default Shipping Address for User " + userId)
                .orderItems(orderItems)
                .build();
        ...
        log.info("Order created from cart successfully with ID: {} for user ID: {}", savedOrder.getId(), userId);

        // 4. Clear the user's cart after order creation
        try {
            shoppingCartClient.clearCartByUserId(userId);
            log.info("Cart cleared successfully for user ID: {}", userId);
        } catch (Exception e) {
            log.error("Failed to clear cart for user ID: {}. Error: {}", userId, e.getMessage());
    ...
    public List<OrderResponse> getAllOrders() {
        ...
        // Ensure order items are eagerly fetched if needed for all orders (or use DTO mapping)
        // For simplicity, assuming default fetch or that mapToOrderResponse handles lazy loading gracefully
    ...
    private OrderResponse mapToOrderResponse(Order order) {
        // Ensure order.getOrderItems() is not null before streaming
        List<OrderItemResponse> itemResponses = order.getOrderItems() != null ?
                order.getOrderItems().stream()
                        .map(this::mapToOrderItemResponse)
                        .collect(Collectors.toList()) :
                Collections.emptyList(); // Return empty list if no items
